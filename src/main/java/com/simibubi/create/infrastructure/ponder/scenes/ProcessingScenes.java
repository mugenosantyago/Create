...
public class ProcessingScenes {
	...
	public static void mixing(SceneBuilder builder, SceneBuildingUtil util) {
		...
		scene.world().modifyBlockEntityNBT(util.select().position(basin), BasinBlockEntity.class, nbt -> {
			nbt.put("VisualizedItems",
				NBTHelper.writeCompoundList(ImmutableList.of(IntAttached.with(1, purple)), ia -> (CompoundTag) ia.getValue().save(scene.world().getHolderLookupProvider())));
		});
		scene.idle(4);
	...
	public static void compacting(SceneBuilder builder, SceneBuildingUtil util) {
		...
		scene.world().modifyBlockEntityNBT(util.select().position(basin), BasinBlockEntity.class, nbt -> {
			nbt.put("VisualizedItems",
				NBTHelper.writeCompoundList(ImmutableList.of(IntAttached.with(1, copperBlock)), ia -> (CompoundTag) ia.getValue().save(scene.world().getHolderLookupProvider())));
		});
		scene.idle(4);
		...
		scene.world().modifyBlockEntityNBT(util.select().position(basin), BasinBlockEntity.class, nbt -> {
			nbt.put("VisualizedItems",
				NBTHelper.writeCompoundList(ImmutableList.of(IntAttached.with(1, bark)), ia -> (CompoundTag) ia.getValue().save(scene.world().getHolderLookupProvider())));
		});
		scene.idle(4);
	...
	public static void blazeBurner(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.world().modifyBlockEntityNBT(util.select().position(4, 1, 2), teType,
			nbt -> nbt.put("HeldItem", AllItems.BLAZE_CAKE.asStack().save(scene.world().getHolderLookupProvider())));
		...
		scene.world().modifyBlockEntityNBT(util.select().position(basinPos), BasinBlockEntity.class, nbt -> {
			nbt.put("VisualizedItems",
				NBTHelper.writeCompoundList(ImmutableList.of(IntAttached.with(1, new ItemStack(Blocks.BRICKS))),
					ia -> (CompoundTag) ia.getValue().save(scene.world().getHolderLookupProvider())));
		});
		scene.idle(4);
